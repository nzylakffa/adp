...
def scrape_espn_adp():
    url_2 = "https://www.fantasypros.com/nfl/adp/half-point-ppr-overall.php"
    ...
    df_2 = df_2[["Player", "Pos", "Sleeper", "Yahoo"]]
    ...
    df['Average ADP'] = (df['ESPN'] + df['Yahoo'] + df['Underdog'] + df['Sleeper'])/4
    # Sort by Average ADP
    df.sort_values(by='Average ADP', inplace=True)
    # Best ADP
    df['Best ADP Site'] = np.where((df['ESPN'] >= df['Yahoo']) & (df['ESPN'] >= df['Underdog']) & (df['ESPN'] >= df['Sleeper']), "ESPN",
                                  (np.where((df['Yahoo'] >= df['ESPN']) & (df['Yahoo'] >= df['Underdog']) & (df['Yahoo'] >= df['Sleeper']),'Yahoo',
                                                     (np.where((df['Sleeper'] >= df['ESPN']) & (df['Sleeper'] >= df['Yahoo']) & (df['Sleeper'] >= df['Underdog']),'Sleeper', 'Underdog')))))
    
    
    
    # Worst ADP
    df['Worst ADP Site'] = np.where((df['ESPN'] <= df['Yahoo']) & (df['ESPN'] <= df['Underdog']) & (df['ESPN'] <= df['Sleeper']), "ESPN",
                                  (np.where((df['Yahoo'] <= df['ESPN']) & (df['Yahoo'] <= df['Underdog']) & (df['Yahoo'] <= df['Sleeper']),'Yahoo',
                                                     (np.where((df['Sleeper'] <= df['ESPN']) & (df['Sleeper'] <= df['Yahoo']) & (df['Sleeper'] <= df['Underdog']),'Sleeper', 'Underdog')))))
    ...
    df = df[["Average ADP", "Player", "Team", "Pos", "Final Pos Rank", "ESPN", "Yahoo", "Underdog", "Sleeper", "Best ADP Site", "Worst ADP Site"]]
...
if st.checkbox("Get ADP's"):
    ...
    if all_players == True:
        ...
        best_values = st.multiselect("Best Values by Site",
                                    ["ESPN", "Yahoo", "Underdog", "Sleeper"], ["ESPN", "Yahoo", "Underdog", "Sleeper"])
        ...
        adp_df['Yahoo'] = round(adp_df['Yahoo'],2)
        # Rename Columns
        ...
        st.dataframe(adp_df.style.text_gradient(cmap=cm).format({'Average ADP':'{:.1f}',
                                                                'Sleeper':'{:.0f}',
                                                                'Yahoo':'{:.0f}'}).apply(highlight_rows, axis=1))
    else:
        filtered = st.multiselect("Show Only Select Positions",
                                  ["QB", "RB", "WR", "TE", "DST"], ["QB", "RB", "WR", "TE"])
        adp_df = adp_df[adp_df['Pos'].isin(filtered)]
